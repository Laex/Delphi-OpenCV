...
Type TCvSubdiv2DPointLocation = Integer;

const
{ CvSubdiv2DPointLocation enum }
  CV_PTLOC_ERROR = -2; CV_PTLOC_OUTSIDE_RECT = -1; CV_PTLOC_INSIDE = 0; CV_PTLOC_VERTEX = 1; CV_PTLOC_ON_EDGE = 2;

Type TCvNextEdgeType = Integer;

const
{ CvNextEdgeType enum }
  CV_NEXT_AROUND_ORG = $00; CV_NEXT_AROUND_DST = $22; CV_PREV_AROUND_ORG = $11; CV_PREV_AROUND_DST = $33;
CV_NEXT_AROUND_LEFT = $13; CV_NEXT_AROUND_RIGHT = $31; CV_PREV_AROUND_LEFT = $20; CV_PREV_AROUND_RIGHT = $02;
...
Type pCvStereoGCState = ^TCvStereoGCState;

TCvStereoGCState = record Ithreshold: Integer; interactionRadius: Integer; K, lambda, lambda1, lambda2: Single;
occlusionCost: Integer; minDisparity: Integer; numberOfDisparities: Integer; maxIters: Integer; left: pCvMat;
right: pCvMat; dispLeft: pCvMat; dispRight: pCvMat; ptrLeft: pCvMat; ptrRight: pCvMat; vtxBuf: pCvMat;
edgeBuf: pCvMat; end;
...
(* We discriminate between foreground and background pixels
  ...
  // *)
//
const CV_BG_MODEL_FGD = 0; CV_BG_MODEL_MOG = 1; // * "Mixture of Gaussians".	*)
CV_BG_MODEL_FGD_SIMPLE = 2;

Type

  ppCvBGStatModel = ^pCvBGStatModel; pCvBGStatModel = ^TCvBGStatModel;

// typedef void (CV_CDECL * CvReleaseBGStatModel)( struct CvBGStatModel** bg_model );
TCvReleaseBGStatModel =
procedure(Var bg_model: pCvBGStatModel); cdecl;
// typedef int (CV_CDECL * CvUpdateBGStatModel)( IplImage* curr_frame, struct CvBGStatModel* bg_model, double learningRate );
TCvUpdateBGStatModel =
function(curr_frame: pIplImage; bg_model: pCvBGStatModel; learningRate: double): Integer; cdecl;

TCvBGStatModel = record _type: Integer; // *type of BG model
release: TCvReleaseBGStatModel; update: TCvUpdateBGStatModel; background: pIplImage; // *8UC3 reference background image
...
// Default parameters of foreground detection algorithm:
  CV_BGFG_FGD_LC = 128; CV_BGFG_FGD_N1C = 15; CV_BGFG_FGD_N2C = 25;

CV_BGFG_FGD_LCC = 64; CV_BGFG_FGD_N1CC = 25; CV_BGFG_FGD_N2CC = 40;
...
CV_BGFG_FGD_ALPHA_3 = 0.1; CV_BGFG_FGD_DELTA = 2; CV_BGFG_FGD_T = 0.9; CV_BGFG_FGD_MINAREA = 15;
...
(* See the above-referenced Li/Huang/Gu/Tian paper
  ...
  // *)
Type pCvFGDStatModelParams = ^TCvFGDStatModelParams;

TCvFGDStatModelParams = record Lc: Integer;
...
(* Note:  "MOG" == "Mixture Of Gaussians": *)
  const CV_BGFG_MOG_MAX_NGAUSSIANS = 500;
...
CV_BGFG_MOG_WEIGHT_INIT = 0.05; CV_BGFG_MOG_SIGMA_INIT = 30; CV_BGFG_MOG_MINAREA = 15;

CV_BGFG_MOG_NCOLORS = 3;

type pCvGaussBGStatModelParams = ^TCvGaussBGStatModelParams;

TCvGaussBGStatModelParams = record win_size: Integer; // * = 1/alpha
n_gauss: Integer; bg_threshold, std_threshold, minArea: double; weight_init, variance_init: double; end;

pCvGaussBGValues = ^TCvGaussBGValues;

TCvGaussBGValues = record match_sum: Integer; weight: double; variance: array [0 .. CV_BGFG_MOG_NCOLORS - 1] of double;
mean: array [0 .. CV_BGFG_MOG_NCOLORS - 1] of double; end;

pCvGaussBGPoint = ^TCvGaussBGPoint;

TCvGaussBGPoint = record g_values: pCvGaussBGValues; end;
...
storage: pCvMemStorage; // storage for foreground_regions
foreground_regions: pCvSeq; // foreground object contours
params: TCvGaussBGStatModelParams; g_point: pCvGaussBGPoint; countFrames: Integer; mog: pointer; end;
...
type pCvBGCodeBookElem = ^TCvBGCodeBookElem;

TCvBGCodeBookElem = record next: pCvBGCodeBookElem; tLastUpdate: Integer; stale: Integer;
boxMin: array [0 .. 2] of byte; boxMax: array [0 .. 2] of byte; learnMin: array [0 .. 2] of byte;
learnMax: array [0 .. 2] of byte; end;

pCvBGCodeBookModel = ^TCvBGCodeBookModel;

TCvBGCodeBookModel = record size: TCvSize; T: Integer; cbBounds: array [0 .. 2] of byte; modMin: array [0 .. 2] of byte;
modMax: array [0 .. 2] of byte; cbmap: pCvBGCodeBookElem; storage: pCvMemStorage; freeList: pCvBGCodeBookElem; end;
...
function cvSubdiv2DEdgeOrg(edge: TCvSubdiv2DEdge): pCvSubdiv2DPoint; inline; Var e: pCvQuadEdge2D; begin
// CvQuadEdge2D* e = (CvQuadEdge2D*)(edge & ~3);
e := pCvQuadEdge2D(edge and (not 3));
// return (CvSubdiv2DPoint*)e->pt[edge & 3];
Result := pCvSubdiv2DPoint(e^.pt[edge and 3]); end;

function cvSubdiv2DEdgeDst(edge: TCvSubdiv2DEdge): pCvSubdiv2DPoint; Var e: pCvQuadEdge2D; begin
// CvQuadEdge2D* e = (CvQuadEdge2D*)(edge & ~3);
e := pCvQuadEdge2D(edge and (not 3));
// return (CvSubdiv2DPoint*)e->pt[(edge + 2) & 3];
Result := pCvSubdiv2DPoint(e^.pt[(edge + 2) and 3]); end;

function cvSubdiv2DLocate; external legacy_lib;

function cvSubdiv2DGetEdge(edge: TCvSubdiv2DEdge; _type: TCvNextEdgeType): TCvSubdiv2DEdge; Var e: pCvQuadEdge2D; begin
// CvQuadEdge2D* e = (CvQuadEdge2D*)(edge & ~3);
e := pCvQuadEdge2D(edge and (not 3));
// edge = e->next[(edge + (int)type) & 3];
edge := e^.next[(edge + _type) and 3];
// return  (edge & ~3) + ((edge + ((int)type >> 4)) & 3);
Result := (edge and (not 3)) + ((edge + (_type shr 4)) and 3); end;

function cvSubdiv2DRotateEdge(edge: TCvSubdiv2DEdge; rotate: Integer): TCvSubdiv2DEdge; begin
// return  (edge & ~3) + ((edge + rotate) & 3);
Result := (edge and (not 3)) + ((edge + rotate) and 3); end;
